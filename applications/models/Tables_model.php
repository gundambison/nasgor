...
*/
	function saveData($data)
	{
		...
		//==========TAMBAHKAN STATUS
		$iTabStat=dbId();
		$data2=array('tabstat_id'=>$iTabStat,'tabstat_tables'=>$id,'tabstat_status'=>1);
		$this->db->insert($this->tableRel,$data2);
		$str = $this->db->last_query();			 
		logConfig("create Relation:$str",'logDB');
		$iTabStat=dbId();
		$data2=array('tabstat_id'=>$iTabStat,'tabstat_tables'=>$id,'tabstat_status'=>2);
		$this->db->insert($this->tableRel,$data2);
		$str = $this->db->last_query();			 
		logConfig("create Relation:$str",'logDB');
		...
		for($i=1;$i<=10;$i++){
			$name1=sprintf("%sfield%03s",$prefix,$i);
			$fields[$name1]=array( 
			    'type' => 'VARCHAR',  
			    'constraint' => '100');
		}
			...
			$this->dbforge->add_key($prefix.'_name');
			$this->dbforge->add_key($prefix.'_code');
		for($i=1;$i<=10;$i++){
			$this->dbforge->add_key(sprintf("%sfield%03s",$prefix,$i));
		}
