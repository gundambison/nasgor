...
class Menu_model extends CI_Model {
	...
	function updateSubMenu($parent, $showid){
		$sql="update {$this->table} set menu_show=$showid where menu_parent=$parent";
		$this->db->query($sql);
		return true;
	}
		...
		foreach($parents as $parent){
			...
			if(count($submenu)!=0){
				foreach($submenu as $menu){
					if($this->member->getPermision($userid,$menu['show'])){
						$icon=$menu['icon'];
...
*/	
	function getData($id,$field='menu_id')
	{
		...
		if($field=='menu_id'){
			$data=dbFetchOne($sql);
			$data['total']=$this->totalAllSub($data['id']);
		}else{ 
			$result=dbQuery($sql,0);
			$data=array();
			$i=0;
			foreach ($result->result_array() as $row){
				$row['total']=$this->totalAllSub($row['id']);
				$data[]=$row;
			}
		}
		return $data;
