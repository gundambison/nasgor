...
public $tableupermit='mujur_userpermit';
	function synch(){
		$table=$this->userjomla->table;
		$sql="SELECT username,name  FROM `{$this->table}` right join $table on  user_name=username
...
*/
	function updateDataPermision($data,$id)
	{
		dbIdReport('update','update User permision',json_encode($data), 100);
		$sql="delete from {$this->tableupermit} where uper_user=$id";
		dbQuery($sql);
		foreach($data as $permit){
			$dtInsert=array('uper_user'=>$id, 'uper_permision'=>$permit);
			$dtInsert['uper_id']=dbId();
			$dt=$this->permision->getData($permit);
			$dt2=$this->getData($id);
			$dtInsert['uper_detail']=$dt2['name'].";".$dt['name'];
			//getData(
			$this->db->insert($this->tableupermit,$dtInsert);
		}
		logConfig("update User permision",'logDB');
