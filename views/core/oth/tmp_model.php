...
*/	
	function totalAll()
	{
	    ...
	    public function __construct()
        {
			...
			$data=dbFetchOne($sql);
			if(count($data)!=0){
			  if(!isset($data['<?=$prefix;?>created'])){
				$sql="ALTER TABLE `{$this->table}` ADD `<?=$prefix;?>created` DATETIME NOT NULL ;";
				dbQuery($sql,1);
			  }else{}
			  if(!isset($data['<?=$prefix;?>modified'])){
				$sql="ALTER TABLE `{$this->table}` ADD `<?=$prefix;?>modified` TIMESTAMP on update CURRENT_TIMESTAMP NOT NULL;";
				dbQuery($sql,1);
			  }else{}
			}
        }
