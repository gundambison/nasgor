...
<!--edit this manualy-->
 ...
 echo $title;?>" class="easyui-datagrid" style="width:700px;height:450px"
	...
	<th field='date' width='90'>TANGGAL</th>			 
			</tr>
		</thead>
	</table>
	<div id="toolbar">
...
function updateForm()
{
	...
	console.log(datax);
	var request = $.ajax({
          url: "<?php echo my_url();?>kursAction/update",
          type: "POST",
          data: datax,
          dataType: "json"
    });
...
function newData()
{
	...
	console.log(datax);
	var request = $.ajax({
          url: "<?php echo my_url();?>kursAction/new",
          type: "POST",
          data: datax,
          dataType: "json"
    });
...
function newform(){
	createWin();
	url="<?php echo my_url();?>kursForm/baru";
...
function edit(){
	...
	if (row){
		createWin(); 
				url="<?php echo my_url();?>kursForm/edit/"+row.id;
				$('#win').window('refresh', url);
				$('#win').window('open');
...
function view(){
	...
	if (row){
		createWin();
				...
				url="<?php echo my_url();?>kursForm/view/"+row.id;
				$('#win').window('refresh', url);
				$('#win').window('open');
